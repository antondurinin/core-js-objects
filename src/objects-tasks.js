...
function mergeObjects(objects) {
  return objects.reduce((acc, obj) => {
    ...
    return { ...acc, ...Object.fromEntries(newEntries) };
  }, {});
}
...
function sellTickets(queue) {
  ...
  const result = queue.reduce((canSell, bill) => {
...
const cssSelectorBuilder = {
  ...
  validateOrder(currentOrder, newOrder) {
    const sequence = [
      'element',
      'id',
      'class',
    ...
    if (
      sequence.indexOf(newOrder) <
      Math.max(...currentOrder.map((order) => sequence.indexOf(order)))
    ) {
      throw new Error(
  ...
  createSelector(method, value) {
    const builder = Object.create(this);
    builder.selector = this.selector;
    builder.order = this.order ? [...this.order] : [];
    builder.used = { ...this.used };
    builder.validateOrder(builder.order, method);
    builder.selector += value;
    builder.order.push(method);
    return builder;
  },

  element(value) {
    const builder = this.createSelector('element', value);
    builder.validateUsage('element');
    return builder;
  },

  id(value) {
    const builder = this.createSelector('id', `#${value}`);
    builder.validateUsage('id');
    return builder;
  },
  ...
  pseudoElement(value) {
    const builder = this.createSelector('pseudoElement', `::${value}`);
    builder.validateUsage('pseudoElement');
    return builder;
  },
